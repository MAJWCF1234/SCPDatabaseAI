...
class SCPDatabaseApp:
    def __init__(self, root):
        ...
        self.main_frame = tk.Frame(root, padx=10, pady=10, bg='#1c1c1c') # Dark background
        ...
        self.message_entry = tk.Entry(self.main_frame, width=100, bg='#2d2d2d', fg='#00ff00', insertbackground='white') # Darker input field
        self.message_entry.pack(fill='x', pady=(0, 10))
        self.message_entry.bind("<Return>", self.send_message_event)

        # Thematic button for message sending
        self.send_message_button = tk.Button(self.main_frame, text="Send Query", command=self.send_message, bg='#333333', fg='#00ff00') # Dark button
        ...
        self.api_key_entry = tk.Entry(self.api_key_frame, width=100, show="*", bg='#2d2d2d', fg='#00ff00', insertbackground='white') # Dark input field
        self.api_key_entry.pack(side=tk.LEFT, expand=True, fill='x')
        self.set_api_key_button = tk.Button(self.api_key_frame, text="Authorize", command=self.set_api_key, bg='#333333', fg='#00ff00') # Dark button
        self.set_api_key_button.pack(side=tk.RIGHT)

        # Initially disable the send message button to enforce API key setup
    ...
    def get_scp_response(self, message):
        if not self.api_key:
        ...
        try:
            response = openai.ChatCompletion.create(
                model="gpt-4",
                messages=[
                    {"role": "system", "content": "SCP Foundation database interface is now active."},
